...
type ParsePrintFormat<T extends string, Pre extends string = '', Result extends ControlsMap[keyof ControlsMap][] = []> = T extends `${infer Sign}${infer Type}${infer R}`
  ? Sign extends '%'
    ? Pre extends '%'
      ? ParsePrintFormat<`${Type}${R}`, '', Result>
      : Type extends keyof ControlsMap
        ? ParsePrintFormat<R, Type, [...Result, ControlsMap[Type]]>
        : ParsePrintFormat<`${Type}${R}`, Sign, Result>
    : ParsePrintFormat<`${Type}${R}`, Sign, Result>
  : Result;
