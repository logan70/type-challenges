...
type MergeObject<O> = Omit<O, never>

type PartialByKeys<T, K extends (string | number | symbol) = keyof T> = MergeObject<Omit<T, K> & {
  [Key in Extract<K, keyof T>]?: T[Key]
}>


type PartialByKeys2<T, K extends (string | number | symbol) = keyof T> = MergeObject<
