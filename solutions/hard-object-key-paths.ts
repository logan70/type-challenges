...
type GenPath<K extends string | number,IsRoot extends boolean> = IsRoot extends true? `${K}`: `.${K}` | (K extends number? `[${K}]` | `.[${K}]`:never)

type ObjectKeyPaths<T extends object, IsRoot extends boolean = true, K extends keyof T = keyof T> = K extends string | number
  ? T[K] extends object
    ? GenPath<K, IsRoot> | `${GenPath<K, IsRoot>}${ObjectKeyPaths<T[K], false>}`
    : GenPath<K, IsRoot>
  : never;
