...
type StoreOption<State, Getters, Actions,
  GettersThis
> = {
  id: string;
  state: () => State;
  getters: Getters & ThisType<Readonly<State> & GettersThis>;
  actions: Actions & ThisType<State & Readonly<GettersThis> & Actions>
}
type StoreReturn<State, Getters, Actions, GettersThis> = State & GettersThis & Actions
...
declare function defineStore<
  ...
  Actions,
  GettersThis = {
    [K in keyof Getters]: Getters[K] extends (...args: any[]) => infer R ? R : never
  }
>(store: StoreOption<State, Getters, Actions, GettersThis>): StoreReturn<State, Getters, Actions, GettersThis>
