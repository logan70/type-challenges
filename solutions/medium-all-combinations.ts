...
type AllCombinations<S extends string, Prefix extends string = '', Val extends string = ''> = S extends `${infer F}${infer R}`
  ? (`${Val}${F}` | AllCombinations<`${Prefix}${R}`, '', `${Val}${F}`> | AllCombinations<`${R}`, `${Prefix}${F}`, `${Val}`>)
  : ''

type AllCombinations2<
  S extends string,
  PRE extends string = ''
