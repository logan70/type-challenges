...
type UnionToIntersectionFn<T> = (T extends any ? (t: () => T) => any : never) extends (t: infer R) => any
  ? R
  : never

type LastOfUnion<T> = UnionToIntersectionFn<T> extends () => infer R ? R : never;

type UnionToTuple<T, K = LastOfUnion<T>> = [T] extends [never]
  ? []
