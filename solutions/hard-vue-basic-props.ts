...
type ExtractPropType<T> = T extends { type: infer P }
  ? ExtractPropType<P>
  : T extends any[]
    ? ExtractPropType<T[number]>
    : {} extends T
      ? any
      : T extends { (...args: any[]): infer Q }
        ? Q
        : T extends { new(...args: any[]): infer J }
          ? J
          : T;


type VueOptions<Props, Data, Computed, Methods,
  PropsThis = {
    [K in keyof Props]: ExtractPropType<Props[K]>
  },
  ComputedThis = {
    [K in keyof Computed]: Computed[K] extends () => infer R ? R : never;
  },
> = {
  data: (this: PropsThis) => Data,
  methods: Methods & ThisType<PropsThis & Data & ComputedThis & Methods>,
  computed: Computed & ThisType<PropsThis & Data & ComputedThis & Methods>,
  props: Props,
};

declare function VueBasicProps<Props, Data, Computed, Methods>(options: VueOptions<Props, Data, Computed, Methods>): any
