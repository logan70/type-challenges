...
type IntersectionToObject<A> = Omit<A, never>

type Enum<T extends readonly string[], N extends boolean = false, Temp extends any[] = []> = T extends readonly [infer F extends string, ...infer R extends readonly string[]]
  ? IntersectionToObject<
      Enum<R, N, [...Temp, any]> &
      {
        readonly [K in Capitalize<F>]: N extends true
          ? Temp['length']
          : F
      }
    >
